...
------------------------------------------------------------------------
    ...
    3 - HYPERPRARMS:
        ...
        quite as well for SpaceInvaders. TODO: Find better parameters
'''
########################################################################
'''
------------------------------------------------------------------------
Imports:
------------------------------------------------------------------------
Import required libraries. Some explanation regarding them
    1 - numpy: for array handling and manipulation
    2 - tensorflow: for neural network building and training
    3 - gym: for Atari game-environment
    4 - scipy: for image data manipulation
    5 - sys: for pushing all the prints to the terminal
    6 - os: for creating file path variables for storing data and checkpoints  
    7 - time: 
    8 - csv: for reading the csv and text files
    9 - argparse: for creating special command line executions
'''
...
Path Variables:
------------------------------------------------------------------------
Here we create file path variables in which we will store all the required
Q-values and Rewards. One of the most important thing we will store is our
checkpoints for the neural network training. File-paths are global variables 
for convenience so they don't have to be passed around between all the objects.
We should first set checkpoint_base_dir, then call the function update_paths(env_name) 
to update all the paths. This should be done before we create the Agent and NeuralNetwork etc.
'''

# Default base-directory for the checkpoints and log-files.
# The environment-name will be appended to this.

checkpoint_base_dir = 'checkpoints_QLearning/'
...
def update_paths(env_name):
    ...
    Call this after we have changed checkpoint_base_dir and
    before we create the Neural Network.
    
    :param env_name:
        Name of the game-environment you will use in OpenAI Gym.
        For example: 
            update_paths('Breakout-v0')
    """
...
Logging:
------------------------------------------------------------------------
Here some requried classes which are reqired for logging data during training.
There is one parent class Log, which will be inherited by reamaning two classes 
i.e., class LogRewards and LogQValues. 
'''
...
Print:
------------------------------------------------------------------------
This function is used to used to push all the prints to the terminal. Some notes on
sys.std.flush and sys.std.out have been attached here in the form of a link. 
sys.std.out is equivalent to print. There is special use for sys.std.flush. 
Read about them more here:
    1 - https://stackoverflow.com/questions/10019456/usage-of-sys-stdout-flush-method
    2 - https://stackoverflow.com/questions/1450551/buffered-vs-unbuffered-io
'''
...
------------------------------------------------------------------------
This function is used to store all the states and actions taken. Buffer has max limit
on how much data it can store. Also we do backward sweep to update our Q values and 
finally we print the statistics of this run. 
'''
...
class ReplayMemory(object):
    ...
    def all_batches(self, batch_size=128):
        ...
        estimate_all_q_values() below. It is written to show an easy and 
        efficient way to loop over all the data in the replay-memory.
        """
    ...
    def estimate_all_q_values(self, model):
        ...
        Note that this function is not currently being used. It is written
        to show an efficient way to iterate over all the states and Q-values.
        :param model:
...
------------------------------------------------------------------------
This function is used to control the learning rate of the neural network or 
epsilon-greedy's probability parameter etc. All the parameter are decreased
linearlly based on nnumber of iterations. 
'''

class LinearControlSignal(object):
    """
...
------------------------------------------------------------------------
This function is used to choose action between random action and action with 
highest Q-value.
'''
...
Neural Network:
------------------------------------------------------------------------
This class creates a Neural Network will be used to estimate Q-Values. Based on the
error of estimation we will train the nerual network. 
'''
...
class NeuralNetwork(object):
    ...
    def __init__(self, num_actions, replay_memory):
        ...
        # convolutional layer. Effect of this is that this resulted in significantly
        ...
        # If we change the architecture of the network, particularly
        # if we add or remove layers, then you may have to change
        ...
        # This means the scale of the gradient will be  different.
        # Hyperparameters here have tuned for old tensorflow API
        # PrettyTensor which used tf.reduce_mean(). So instead we
        # calculate the L2-loss similarly to how it is done in PrettyTensor.
        # TODO: Change it to self.loss = tf.nn.l2_loss(self.q_values - self.q_values_new)
        # TODO: Retune hyperparameters
